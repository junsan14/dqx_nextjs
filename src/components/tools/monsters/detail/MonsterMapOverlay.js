...
export default function MonsterMapOverlay({
  ...
  const content = (
    <div style={styles.mapCard}>
      {areaInfo.all.length > 0 ? (
        <div style={styles.areaBar} title={areaInfo.all.join(", ")}>
          <span style={styles.areaBarLabel}>生息エリア</span>

          <div style={styles.areaChipWrap}>
            {areaInfo.visible.map((label) => (
              <span key={label} style={styles.areaChip}>
                {label}
              </span>
            ))}

            {areaInfo.hiddenCount > 0 ? (
              <span style={styles.areaChipMore}>
                +{areaInfo.hiddenCount}
              </span>
            ) : null}
          </div>
        </div>
